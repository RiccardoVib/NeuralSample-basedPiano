...
def plotResult(pred, tar, model_save_dir, save_folder, title):
    ...
    Plot the rendered results
      ...
      :param model_save_dir: the director where the models are saved [string]
      :param save_folder: the director where the model is saved [string]
      :param title: suffix name for the file [string]
    """
    fs = 24000
    N_stft = 2048
    N_fft = fs * 2
    ...
    librosa.display.specshow(S_db, sr=fs, y_axis='log', x_axis='time', ax=ax[0])
    ...
    S_db = librosa.amplitude_to_db(np.abs(D))#, ref=np.max)
    librosa.display.specshow(S_db, sr=fs, y_axis='log', x_axis='time', ax=ax[1])
    ax[1].set_ylabel('Frequency [Hz]')
    ax[1].set_xlabel('Time [sec]')
    fig.savefig(model_save_dir + '/' + save_folder + '/' + title + 'STFT.pdf', format='pdf')
    plt.close('all')

    Dp = librosa.stft(pred, hop_length=N_stft, n_fft=N_stft)  # STFT of y
    Dt = librosa.stft(tar, hop_length=N_stft, n_fft=N_stft)  # STFT of y

    fig, ax = plt.subplots(1, 1)
    ax.plot(np.angle(Dt).reshape(-1)[:3000], label='Target')
    ax.plot(np.angle(Dp).reshape(-1)[:3000], label='Prediction')
    #ax.set_xlabel('Frequency')
    #ax.set_ylabel('Magnitude (dB)')
    #ax.axis(xmin=20, xmax=22050)

    ax.legend(loc='upper right')
    fig.savefig(model_save_dir + '/' + save_folder + '/' + title + 'phase.png', format='png')
    plt.close('all')
...
def plotTraining(loss_training, loss_val, model_save_dir, save_folder, name):
    ...
    Plot the training against the validation losses
      ...
      :param model_save_dir: the director where the models are saved [string]
      :param save_folder: the director where the model is saved [string]
      :param name: suffix name for the file [string]
...
def predictWaves(predictions, x_test, y_test, model_save_dir, save_folder, fs, title):
    """
...
def render_results(pred, x, y, model_save_dir, save_folder, fs):

    predictions = np.array(pred.reshape(-1))
    predictWaves(predictions, np.array(x, dtype=np.float32).reshape(-1),  np.array(y, dtype=np.float32).reshape(-1), model_save_dir, save_folder, fs, 'S')

...
class MambaBlock(tf.keras.layers.Layer):
    def __init__(self, layer_id, model_input_dims, model_internal_dim, conv_use_bias, dense_use_bias, conv_kernel_size, delta_t_rank, model_states, trainable, **kwargs):
        ...
        self.in_projection = tf.keras.layers.Dense(
            ...
            input_shape=(self.model_input_dims,), use_bias=False, trainable=trainable)
        ...
        self.conv1d = tf.keras.layers.Conv1D(
            ...
            padding='causal', trainable=trainable
        )

        # this layer takes in current token 'x'
        # and outputs the input-specific Δ, B, C (according to S6)
        self.x_projection = tf.keras.layers.Dense(self.delta_t_rank + self.model_states * 2, use_bias=False, trainable=trainable)
        ...
        self.delta_t_projection = tf.keras.layers.Dense(self.model_internal_dim,
                                               input_shape=(self.delta_t_rank,), use_bias=True, trainable=trainable)
        ...
        self.out_projection = tf.keras.layers.Dense(
            ...
            use_bias=self.dense_use_bias, trainable=trainable)
...
class ResidualBlock(tf.keras.layers.Layer):
    def __init__(self, layer_id, model_input_dims, model_internal_dim, conv_use_bias, dense_use_bias, conv_kernel_size, delta_t_rank, model_states, trainable, **kwargs):
        ...
        self.mixer = MambaBlock(layer_id, model_input_dims, model_internal_dim, conv_use_bias, dense_use_bias, conv_kernel_size, delta_t_rank, model_states, trainable)
